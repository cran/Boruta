...
Accumulated hit counts are finally assessed; features which significantly outperform best shadow are claimed confirmed, which these which significantly under-perform best shadow are claimed rejected and removed from the set for all subsequent iterations.
